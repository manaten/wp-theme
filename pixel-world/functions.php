...
add_action( 'after_setup_theme', array('ManatenWp', 'setUp') );
...
class ManatenWp {
	...
	public static function getPageName() {
		if (is_category()) {
			return sprintf('カテゴリアーカイブ: %s', single_cat_title( '', false ) );
		} else if (is_archive()) {
			if ( is_day() ) {
				return sprintf('日別アーカイブ: %s', get_the_date() );
			} elseif ( is_month() ) {
				return sprintf('月別アーカイブ: %s', get_the_date( _x( 'F Y', 'monthly archives date format', 'twentyten' ) ) );
			} elseif ( is_year() ) {
				return sprintf('年別アーカイブ: %s', get_the_date( _x( 'Y', 'yearly archives date format', 'twentyten' ) ) );
			} else {
				return 'ブログアーカイブ';
			}
		} else if (is_search()) {
